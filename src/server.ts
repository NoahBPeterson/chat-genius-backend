...
const typingUsers = new Map<string, Set<number>>();  // key: 'channel:ID' or 'thread:ID', value: Set of typing user IDs
...
function broadcastTypingStatus(type: 'channel' | 'thread', id: number) {
...
app.get('/api/channels/:channelId/threads', authorize(['admin', 'member']), async (req: Request, res: Response) => {
    try {
...
app.get('/api/threads/:threadId/messages', authorize(['admin', 'member']), async (req: Request, res: Response) => {
    try {
            ...
            try {
                ...
                switch (parsed.type) {
                                ...
                                if (parsed.content) {
                                    await client.query(
                                        `INSERT INTO messages (channel_id, user_id, content, thread_id) 
                                         VALUES ($1, $2, $3, $4)`,
                                        [parsed.channelId, decoded.userId, parsed.content, thread.id]
                                    );
                                }
                            ...
                            try {
                                ...
                                const messageResult = await client.query(
                                    `INSERT INTO messages (channel_id, user_id, content, thread_id) 
                                     VALUES ($1, $2, $3, $4) 
                                     RETURNING *`,
                                    [parsed.channelId, decoded.userId, parsed.content, parsed.threadId]
                                );
                    ...
                    case 'typing_start':
                        try {
                            const decoded = jwt.verify(parsed.token, process.env.JWT_SECRET as string) as { userId: number };
                            const contextType = parsed.threadId ? 'thread' : 'channel';
                            const contextId = parsed.threadId || parsed.channelId;
                            ...
                            broadcastTypingStatus(contextType, contextId);
